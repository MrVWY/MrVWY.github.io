...
哈希表是几乎所有语言都会有的一个数据结构，而Go中也有着map的数据结构，但是在遍历map却是无序的，而且在其里面涉及了内存对齐、哈希冲突等知识点，这是值得去了解。
<!-- more -->
### map结构
...
![](http://www.hizjlhi.com/2020/06/20200622172122708.png)  
...
这里有一个点需要注意，在上述evacuated函数加粗的代码，它定义了xy \[2\]evacDst，里面只含2个evacDst，同时结合上面所定义的cell的状态，可以推测在搬迁过程中，把原来的buckets看出X，而后来等量扩容的部分看成Y，如下图所示。 ![](http://www.hizjlhi.com/2020/06/20200626082230923.png) 正因为如此，在搬迁的过程中，由于所计算的地址可能会跟原来的不同，map的遍历是按顺序遍历的，扩容之后有的key就可能被迁移到溢出桶那边，因此map的遍历是无序的。
