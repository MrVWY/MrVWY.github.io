...
在上篇了解完[channel中的锁自旋机制](http://www.hizjlhi.com/archives/176)后，引起了我对sync包中的mutex和rwmutex这两个锁的兴趣。因此这篇文章打算来探秘一下这两种锁的实现。
<!-- more -->
### Mutex
...
func (m \*Mutex) lockSlow() {
   ...
   for {
      ...
      if atomic.CompareAndSwapInt32(&m.state, old, new) {
         ...
         if waitStartTime == 0 {
            waitStartTime = runtime\_nanotime() //获取等待时间
         }
         //如果queueLifo为true，将当前goroutine调到queue最前面
         runtime\_SemacquireMutex(&m.sema, queueLifo, 1)
         starving = starving  runtime\_nanotime()-waitStartTime > starvationThresholdNs //当前waitStartTime是否大于1ms
         old = m.state
...
func (m \*Mutex) unlockSlow(new int32) {
   ...
   if new&mutexStarving == 0 {
      ...
      for {
         ...
         if atomic.CompareAndSwapInt32(&m.state, old, new) {
            runtime\_Semrelease(&m.sema, false, 1)
            return
         }
         old = m.state
      }
      ...
      runtime\_Semrelease(&m.sema, true, 1)
   }
}

总结 sync.Mutex的lock和unlock理解起来还是有点难度，其中涉及了很多位操作来判断当前mutex出于哪一种状态来做出相应的操作。因此总的来说要慢慢理解，有空时我会更新一副大致流程图出来，方便理解。