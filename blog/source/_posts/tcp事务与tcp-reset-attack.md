...
目前time\_wait状态截断只要是作用于T/TCP协议中。具体还是看图理解 ![](http://www.hizjlhi.com/2020/09/20200903160427835.png) 由上图可知在客户与服务器之间的交互的过程中，比较TCP/IP协议，其多发送了FIN。 这个time\_wait状态截断主要发生在上图的新连接主动打开这块，可以明显看出time\_wait直接被截。仔细观察结合卷3的解释，出现这情况是因此T/TCP新增的字段CC大于上一条连接(8>6)，于是就导致了老的连接关闭，新的连接开始建立。
